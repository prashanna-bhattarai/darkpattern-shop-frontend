    ...
    >
      <figure className="relative h-48 overflow-hidden">
        ...
        {product.urgencyBadge && (
          <span className="absolute top-2 left-2 badge badge-error text-white font-semibold">
            {product.urgencyBadge}
          </span>
        )}
        {product.lowStockBadge && (
          <span className="absolute top-2 right-2 badge badge-warning font-semibold">
            {product.lowStockBadge}
          </span>
        )}
      </figure>
      <div className="card-body p-4">
        ...
        <div className="flex items-center gap-1 text-sm text-base-content/70">
          ...
          {product.reviewCount > 0 && <span>({product.reviewCount.toLocaleString()})</span>}
        </div>
        ...
        {product.requiresAccountToViewPrice ? (
          <p className="text-sm font-semibold text-primary">Sign up to reveal price</p>
        ) : (
        ...
        {product.saleEndsAt && (
          <CountdownTimer targetDate={product.saleEndsAt} className="text-xs text-error" />
        )}
      </div>
    </Link>
  );
